...
class ReservationEditPage extends StatelessWidget {
  final ReservationModel model;
  final Timestamp postTimestamp;
  const ReservationEditPage({super.key, required this.model, required this.postTimestamp});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ReservationEditForm(model: model, postTimestamp: postTimestamp,),
    );
  }
}

class ReservationEditForm extends StatefulWidget {
  final ReservationModel model;
  final Timestamp postTimestamp;
  const ReservationEditForm({super.key, required this.model, required this.postTimestamp});

  @override
  State<ReservationEditForm> createState() => _ReservationEditFormState(model: model, postTimestamp: postTimestamp);
}

class _ReservationEditFormState extends State<ReservationEditForm> {
  final ReservationModel model;
  final Timestamp postTimestamp;
  _ReservationEditFormState({required this.model, required this.postTimestamp});
  ...
  Future<void> _setInitDatas() async {
    setState(() {
      _selectedDay = model.reservationDate.toDate();
      _focusedDay = model.reservationDate.toDate();
      _buttonClicked = DateFormat('HH:mm').format(postTimestamp.toDate());
      _detailsController.text = model.details.toString();

      for(int i = 0; i<_subjects.length; i++) {
        if(_subjects[i] == model.subject) _subjectIndex = i;
      }
    });
  }
  ...
  bool _checkReservedDateTime(String time) {
    for(DateTime date in _reservedDatetimes) {
      ...
      _focusedDay.year == date.year &&
        ...
        if(tempTimeList.contains(time) && time != _buttonClicked) return true;
      }
    }
    return false;
  }
